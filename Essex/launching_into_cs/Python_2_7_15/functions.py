...
# The program starts in defining some important functions that will be used troughout the program.
...
# 2. insertion takes a file, a dictionary, a userinput (already splitted in key and value section) and uses the existing file to open it and to use the append modus (i.e. expand the file modus). The writer option of the csv library creates an writer object.
...
def insertion(filename, dictionary, key, value):
    ...
    # list1 is a temporary list that will bes used for bringing keys and values in the right order so that it can be written accurately into the csv file (key1 first, then key2, value1, value2, etc.).
    ...
    with open(filename + '.csv', 'ab',) as output:
        writer = csv.writer(output, delimiter = ',')
        writer.writerow(list1)
    return dictionary
    
# 3. deletion takes a CSV file, a dictionary and a userinput (a key only) as input. The function then loops over all keys and values in the dictionary and checks if the tuple is in the dictionary. If so, the particular item will be added (to a temporary list called "list1").
        ...
        if list1 != []:
            ...
            for k,v in dictionary.items():
                ...
                for value in v:
                    list2.append(value)
                if counter == 0:
                    with open(filename + '.csv', 'wb',) as output:
                        writer = csv.writer(output, delimiter=",")
...
# 4. search also takes a dictionary and a userinput (a beer name) as arguments and then simply checks for every key and value in the dictionary if there is one key that is equal to given tuple. If so, the value of this key will be displayed ...
...
# 5. An interesting part is when the user wants to sort for items. The function takes a dictionary, a boolean value (to differ key (0) from value (1)), an item number (i.e. which index in the key/values should be chosen) and an order (also boolean, asc = False,  desc = True) as inputs. Depending on the input, the programs checks for the different scenarios. 
# For every sorting procedure, it uses the sorted function from python which uses the timsort algorithm which is a combination of binary insertion sort and merge sort and therefore decides depending on the size of the dictionary which algorithm to choose. 
...
# The argument "key=lambda" simply means that this process should be done in using a lambda function which doesn't save the whole process as an object and it therefore only wants that this whole process will be done in one step. 
def sort(dictionary,boolean_keyORvalue, item_number, order=True):
    sortdic = []
    if order == True:
        for key, value in sorted(dictionary.items(), key=lambda item: item[boolean_keyORvalue][item_number], reverse = True):
            sortdic.append([key,value])
    else:
        for key, value in sorted(dictionary.items(), key=lambda item: item[boolean_keyORvalue][item_number], reverse = False):
            sortdic.append([key,value])
    return sortdic