...
# Display a welcome message and use a blank line before to emphasize the text.
...
while quit == False:
    ...
    # Now, the program goes into a conditional jump, where it tests which input has been given, "insert", "delete", "search" or "sort". Notice, there is also an if-clause for "datacreation" and "testyourapp" but this should only be known to the developer side. The user shouldn't use it as an input. 
    ...
    # third check of the conditional jump checks for the userinput of "sort". If it is correct, then it still asks for two auestions, 1) which category, name, region, city or rating? and 2) ascending or descending?
        ...
        # Still into the sort-clause, the program differentiates between the different inputs that the user can provide (category, order). Depending on this, the sort function will be called differentely. 
