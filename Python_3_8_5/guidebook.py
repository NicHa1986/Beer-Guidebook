...
while quit == False:
    ...
    # third check of the conditional jump checks for the userinput of "sort". If it is correct, then it still asks for two more questions, 1) which category, name, region, city or rating? and 2) ascending or descending?
        ...
        # Still into the sort-clause, the program differentiates between the different inputs that the user can provide (category, order). Depending on this, the sort function will be called differently. 
